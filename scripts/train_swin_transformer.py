#!/usr/bin/env python
# CIFAR-100 Image Classification Project - Swin Transformer Main Implementation
...
from src.models.swin_transformer import SwinTransformerModel
...
def create_model(model_size='tiny', num_classes=100, pretrained=False):
    """
    Create a Swin Transformer model with specified size
    
    Args:
        model_size: Size of the Swin Transformer model ('tiny', 'small', 'base')
        num_classes: Number of output classes
        pretrained: Whether to use pretrained weights
        
    Returns:
        model: The Swin Transformer model
    """
...
def warmup_lr_schedule(optimizer, epoch, warmup_epochs, base_lr):
...
def train_epoch(model, train_loader, criterion, optimizer, device, warmup_epochs=0, epoch=0, base_lr=1e-3, grad_clip=0.0):
    """
    ...
    Args:
        ...
        base_lr: Base learning rate for warmup
    ...
    if epoch < warmup_epochs:
        warmup_lr_schedule(optimizer, epoch, warmup_epochs, base_lr)
...
def train_model(model, train_loader, val_loader, config, device):
    ...
    model_name = str(config.get('model_name', 'swin_transformer'))
    ...
    print(f"  scheduler: {scheduler_name} ({type(scheduler_name)})")
    ...
    if scheduler_name == 'cosine':
        ...
        step_size = int(config.get('step_size', num_epochs // 3))
    ...
    for epoch in range(num_epochs):
        ...
        train_loss, train_acc = train_epoch(
            ...
            warmup_epochs=warmup_epochs, epoch=epoch, base_lr=learning_rate, grad_clip=grad_clip
        )
...
def evaluate_model(model, test_loader, classes, config, device):
    ...
    # Plot confusion matrix (subset for visibility)
    plt.figure(figsize=(12, 10))
    
    # Show confusion matrix for first 20 classes
    cm_subset = cm[:20, :20]
    classes_subset = classes[:20]
    
    sns.heatmap(cm_subset, annot=True, fmt='d', cmap='Blues',
                xticklabels=classes_subset, yticklabels=classes_subset)
    plt.title(f'Confusion Matrix (First 20 Classes) - {model_name}')
...
def plot_training_history(history, config):
    ...
    plt.subplot(2, 2, 2)
    plt.plot(history['train_acc'], label='Training', color='blue')
    ...
    plt.subplot(2, 2, 4)
    plt.bar(range(len(history['time_per_epoch'])), history['time_per_epoch'], color='orange')
    ...
    plt.tight_layout()
    plt.savefig(f"results/plots/{model_name}_training_history.png", dpi=300)
    plt.close()
...
def main():
    """Main function"""
    parser = argparse.ArgumentParser(description='Train and evaluate Swin Transformer models on CIFAR-100')
    parser.add_argument('--config', type=str, default='configs/swin_transformer_config.yaml',
    ...
    config['model_name'] = f"swin_{args.model_size}"
    if args.pretrained:
        config['model_name'] += "_pretrained"

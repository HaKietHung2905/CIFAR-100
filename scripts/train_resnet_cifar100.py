#!/usr/bin/env python
# CIFAR-100 Image Classification Project - ResNet Main Implementation
...
def create_model(model_depth=18, num_classes=100, pretrained=True):
    """
    Create a ResNet model with specified depth
    
    Args:
        model_depth: Depth of the ResNet model (18, 34, or 50)
    ...
    if model_depth == 18:
        ...
        model = create_resnet34(num_classes=num_classes, pretrained=pretrained)
    elif model_depth == 50:
        model = ResNet50Model(num_classes=num_classes, pretrained=pretrained)
    else:
        raise ValueError(f"ResNet{model_depth} not supported. Use 18, 34, or 50.")
...
def train_model(model, train_loader, val_loader, config, device):
    ...
    model_name = str(config.get('model_name', 'resnet'))
...
def main():
    """Main function"""
    parser = argparse.ArgumentParser(description='Train and evaluate ResNet models on CIFAR-100')
    parser.add_argument('--config', type=str, default='configs/resnet_config.yaml',
                        help='Path to configuration file')
    parser.add_argument('--model-depth', type=int, default=18, choices=[18, 34, 50],
                        help='ResNet model depth (18, 34, or 50)')
    ...
    config['model_depth'] = args.model_depth
    config['batch_size'] = args.batch_size
    config['pretrained'] = args.pretrained
    config['model_name'] = f"resnet{args.model_depth}"
    if args.pretrained:
    ...
    # Create model
    model = create_model(
        model_depth=int(config['model_depth']),
        num_classes=100,
